...
public class PairwiseLearner extends Learner {
  ...
  private void createDifferenceSets(Map<String, Map<String, Double[]>> features, 
		  ...
		  Map<String, Map<Pair<String,String>, Instance>> pos,
		  Map<String,Map<Pair<String,String>, Instance>> neg) {
	  
	  for (String query : features.keySet()) {
		  Set<String> docs = features.get(query).keySet();
		  List<Pair<String, String>> pairs = makeAllPairs(docs, query, relMap);
		  Map<Pair<String, String>, Instance> posDocPairsInstances = new HashMap<Pair<String, String>, Instance>();
		  Map<Pair<String, String>, Instance> negDocPairsInstances = new HashMap<Pair<String, String>, Instance>();
		  for (Pair<String, String> pair : pairs) {  
			  Instance inst = new DenseInstance(features.get(query).get(pair.getFirst()).length + 1); //instances have as many attributes as features + 1 (for label)
			  ...
			  if (firstScore - secondScore > 0) {
				  inst.setValue(difference.length, "first");
				  posDocPairsInstances.put(pair, inst);
			  } else {
				  inst.setValue(difference.length, "second");
				  negDocPairsInstances.put(pair, inst);
			  }
		  }
  ...
  private void balancePosAndNeg(Map<String, Map<Pair<String,String>, Instance>> pos,
		  Map<String,Map<Pair<String,String>, Instance>> neg, 
  ...
  private void move(Map<String, Map<Pair<String,String>, Instance>> from,
		  Map<String,Map<Pair<String,String>, Instance>> to, 
		  Map<String, Map<String, Double[]>> features, 
	  ...
	  for (int i = 0; i < number; i++) {
		  ...
		  Map<Pair<String, String>, Instance> fromList = from.get(randQuery);
		  if (fromList.keySet().size() > 0) {
			  ...
			  Instance inst = new DenseInstance(features.get(randQuery).get(oPair.getFirst()).length + 1); //instances have as many attributes as features + 1 (for label)
			  ...
			  for (int j = 0; j < difference.length; j++) {
				  inst.setValue(j, difference[j]);
			  }
			  inst.setValue(difference.length, desiredResult);
			  to.get(randQuery).put(newPair, inst); // put it in the new map
		  }
	  }
  }
  
  private int sizeExampleSet(Map<String, Map<Pair<String,String>, Instance>> pos) {
	  int count = 0;
  ...
  /* end extract feature helper functions */
	...
	public Instances extract_train_features(String train_data_file,
		...
		Map<String, Map<Pair<String,String>, Instance>> pos = new HashMap<String, Map<Pair<String,String>, Instance>>();
		Map<String, Map<Pair<String,String>, Instance>> neg = new HashMap<String, Map<Pair<String,String>, Instance>>();;
		...
		for (String query : pos.keySet()) {
			for (Pair<String, String> docPair : pos.get(query).keySet()) {
				Instance inst = pos.get(query).get(docPair);
				dataset.add(inst);
			}
		}
		...
		for (String query : neg.keySet()) {
			for (Pair<String, String> docPair : neg.get(query).keySet()) {
				Instance inst = neg.get(query).get(docPair);
				dataset.add(inst);
			}
		}
	...
	public TestFeatures extract_test_features(String test_data_file,
		...
		for (String query : features.keySet()) {
			...
			for (String url : features.get(query).keySet()) {
				  ...
				  //Here I am skipping setting the value of the class label -- shouldn't matter yet
				  dataset.add(inst);
